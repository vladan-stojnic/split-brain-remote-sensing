...
single_points = np.linspace(0, 100, num = SINGLE_CLUSTERS + 1)
...
x_points = np.linspace(-110, 110, num = int(np.sqrt(DOUBLE_CLUSTERS)) + 1)
y_points = np.linspace(-110, 110, num = int(np.sqrt(DOUBLE_CLUSTERS)) + 1)
...
with open('resources/clusters_lab_grid_single_'+str(SINGLE_CLUSTERS)+'.npy', 'wb') as f:
    np.savez(f, single_clusters)

with open('resources/clusters_lab_grid_double_'+str(DOUBLE_CLUSTERS)+'.npy', 'wb') as f:
